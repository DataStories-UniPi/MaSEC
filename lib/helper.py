...
from kafka_config_c_p_v01 import CFG_CSV_OUTPUT_DIR, CFG_BUFFER_COLUMN_NAMES, CFG_ALIGNMENT_MODE, CFG_DESIRED_ALIGNMENT_RATE_SEC, \
								 CFG_DATASET_NAME, CFG_SAVE_TO_FILE, CFG_SAVE_TO_TOPIC, CFG_EC_CARDINALITY_THRESHOLD, CFG_EC_TEMPORAL_THRESHOLD, \
								 CFG_EC_DISTANCE_THRESHOLD
...
def checkpoint_csv(timeslice, active_patterns, closed_patterns, tag=''):
	'''
	Save Data to CSV File
	'''
	
	for active_tp, closed_tp, tp in zip(active_patterns, closed_patterns, ['mcs', 'mc']):
		path = os.path.join(CFG_CSV_OUTPUT_DIR, 'kafka_{0}_{6}_evolving_clusters_{1}_params_c={3}_t={4}_theta={5}_dataset_{2}.csv'.format(CFG_ALIGNMENT_MODE, tp, CFG_DATASET_NAME, CFG_EC_CARDINALITY_THRESHOLD, CFG_EC_TEMPORAL_THRESHOLD, CFG_EC_DISTANCE_THRESHOLD, tag))
		pd.concat([active_tp, closed_tp]).to_csv(path, index=False, header=True)
	
	path = os.path.join(CFG_CSV_OUTPUT_DIR, 'kafka_aligned_data_{0}_{2}_dataset_{1}.csv'.format(CFG_ALIGNMENT_MODE, CFG_DATASET_NAME, tag))
...
def send_to_kafka_topic(kafka_producer, kafka_topic, timestamp, timeslice):
	...
	kafka_producer.send(kafka_topic, key=key_time, value=timeslice.to_json(orient='records').encode('utf-8'), timestamp_ms=timestamp)
	
	return key_time


def checkpoint_kafka(kafka_producer, timestamp, timeslice, active_patterns, closed_patterns, alignment_res_topic_name, cluster_res_topic_name):
	'''
	Save Data to Kafka Topic
	'''
	# Save Produced Timeslice to Kafka Topic ```CFG_EC_RESULTS_TOPIC_NAME```
	key_time = send_to_kafka_topic(kafka_producer=kafka_producer, kafka_topic=alignment_res_topic_name, timestamp=timestamp, timeslice=timeslice)
	...
	kafka_producer.send(cluster_res_topic_name, key=key_time, value=json.dumps(data).encode('utf-8'), timestamp_ms=timestamp)


def data_output(kafka_producer, timestamp, timeslice, active_patterns, closed_patterns, tag='', **kwargs):
	if CFG_SAVE_TO_FILE:
		checkpoint_csv(timeslice, active_patterns, closed_patterns, tag)
	if CFG_SAVE_TO_TOPIC:
		checkpoint_kafka(kafka_producer, timestamp, timeslice, active_patterns, closed_patterns, **kwargs)


# ########################################################################################################################


def get_points_polygon(clusters, et, timeslices):
	points = timeslices.loc[(timeslices.mmsi.isin(clusters)) & (timeslices.timestamp == et.timestamp())]
	res = points.groupby(pd.to_datetime(points.timestamp, unit='s'), as_index=False, group_keys=False).apply(lambda l: l.geometry.unary_union.convex_hull.buffer(0.0005))
	return res


def ec_convex_hulls(ecs, location_data):
	ec_hulls = ecs.apply(lambda l: shapely.geometry.MultiPolygon([*list(l.geometry), get_points_polygon(l.clusters, l.et, location_data)]), axis=1)
	return gpd.GeoSeries(pd.melt(ec_hulls).dropna().value, crs='epsg:4326')
