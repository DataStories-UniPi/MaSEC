...
from kafka_config_c_p_v01 import CFG_DESIRED_ALIGNMENT_RATE_SEC, CFG_THRESHOLD_MAX_SPEED, CFG_ALIGNMENT_MODE, CFG_INIT_POINTS, CFG_EC_DISTANCE_THRESHOLD, CFG_EC_CARDINALITY_THRESHOLD, CFG_EC_TEMPORAL_THRESHOLD, CFG_PRODUCER_TIMESTAMP_NAME, CFG_PRODUCER_KEY, CFG_PRODUCER_TIMESTAMP_NAME, CFG_CONSUMER_COORDINATE_NAMES, CFG_CONSUMER_SPEED_NAME
	...
	if not object_buffer.values.shape[0] >= 1: # if this record is the first point of the vessel
		...
		if dt < 1: # if this record is less than 1 sec from the previous
			...
			if dt > 2*CFG_DESIRED_ALIGNMENT_RATE_SEC:
				...
				# tmp = haversine((lon, lat), (object_latest_record[CFG_CONSUMER_COORDINATE_NAMES[0]].values[0], object_latest_record[CFG_CONSUMER_COORDINATE_NAMES[1]].values[0]))*0.539956803 
				# tmp *= 3600/dt
				# print ('---- Vessel Speed {0} m/s | {1} knots | ACK {1} ~ {2}'.format(oid_point_speed, oid_point_speed*1.94384449, tmp))
				...
				if oid_point_speed > CFG_THRESHOLD_MAX_SPEED:
					...
					new_row = {CFG_PRODUCER_KEY: oid, 
								...
								CFG_CONSUMER_COORDINATE_NAMES[0]: lon, 
								CFG_CONSUMER_COORDINATE_NAMES[1]: lat, 
								CFG_CONSUMER_SPEED_NAME: oid_point_speed,
								**kwargs}
					object_pool = object_pool.append(new_row, ignore_index=True)  	# append row to the ```objects_pool``` dataframe		
					
					# Just an experiment in order to accelerate message processing
					if CFG_ALIGNMENT_MODE == 'extra':
						# Adjust Buffer (Keep only X latest positions) -- Query Works for Extrapolation (so far)
						object_pool = object_pool.drop(object_pool.loc[object_pool[CFG_PRODUCER_KEY] == oid].iloc[:-3].index)				
